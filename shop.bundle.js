(() => {
  ...
  function initStorefront({ customCatalog = false } = {}) {
    ...
    document.addEventListener("keydown", (event) => {
      ...
      if (event.key === "Tab" && cartDrawer.classList.contains("is-open")) {
        const focusable = [...cartDrawer.querySelectorAll('button, a[href], input, select, [tabindex="0"]')].filter((element) => !element.disabled && element.getClientRects().length > 0);
  ...
  function initShop() {
    ...
    <article class="shop-product" data-product-id="${product.id}">
      ...
      <div class="shop-product-info"><p class="shop-product-category">${product.category}</p><h3 class="shop-product-name"><button class="product-title-button" type="button" data-quick-view="${product.id}">${product.name}</button></h3><p class="shop-product-tone">${product.tone}</p><div class="shop-product-bottom"><div class="shop-product-price"><strong>${formatBDT(product.price)}</strong>${product.compareAt > product.price ? `<del>${formatBDT(product.compareAt)}</del>` : ""}</div><button class="shop-add" type="button" data-add-to-cart="${product.id}" aria-label="Add ${product.name} to bag"><span class="add-label">Add</span><span aria-hidden="true">+</span></button></div></div>
    </article>`;
    const filterChips = () => {
      const chips = [];
